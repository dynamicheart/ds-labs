...
main(int argc, char **argv)
{
    ...
    int cnt_colors[APP_FLOWS_MAX][3];
    for (i = 0; i < APP_FLOWS_MAX; i++) {
        cnt_send[i] = cnt_pass[i] = 0;
        cnt_colors[i][0] = cnt_colors[i][1] =cnt_colors[i][2] = 0;
    }
    ...
    for (i = 0; i < 10; i++) {
        ...
        for (j = 0; j < burst; j++) {
            ...
            cnt_colors[flow_id][color]++;
        }
        time += 1000000;
    }

    for (i = 0; i < APP_FLOWS_MAX; i++) {
        printf("fid: %d, send: %d, pass: %d, green: %d, yellow: %d, red: %d\n", i, cnt_send[i], cnt_pass[i], cnt_colors[i][0], cnt_colors[i][1], cnt_colors[i][2]);
    }
